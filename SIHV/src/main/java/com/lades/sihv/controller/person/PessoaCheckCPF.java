...
public class PessoaCheckCPF extends AbstractBean {
    ...
    public boolean checkCPF(Cpf objCpf) {
        boolean var = false;
        if (objCpf.getCpf() != null) {
            String strCpf = objCpf.getCpf();
            ...
            if (strCpf.length() == 11) {
                var = MetodoCheckCPF(strCpf);
                if (var) {
//                    getObjMessage().info("CPF Válido!", "");
                    System.out.println("BACK-END WARNING: Método checkCPF [CPF "
                            + "(" + strCpf + ")" + " Válido:" + var + "]");
                } else {
                    getObjMessage().warn("CPF Inválido!", "");
                    objCpf.setCpf("");
                    System.out.println("BACK-END WARNING: Método checkCPF [CPF "
                            + "(" + objCpf.getCpf() + ")" + " Inválido:" + var + "]");
                }
            }
        }
        return var;
