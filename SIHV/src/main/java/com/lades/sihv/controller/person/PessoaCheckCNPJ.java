...
public class PessoaCheckCNPJ extends AbstractBean {
    ...
    private boolean MetodoCheckCNPJ(String CNPJ) {
        boolean var;
        CNPJ = CNPJ.replace(".", "");
        CNPJ = CNPJ.replace("-", "");
        CNPJ = CNPJ.replace("/", "");
...
// Verifica se os dígitos calculados conferem com os dígitos informados.
    ...
    public boolean checkCNPJ(LegalPerson legalPerson) {
        boolean var;
        var = MetodoCheckCNPJ(legalPerson.getCnpj());
        if (var) {
            System.out.println("BACK-END WARNING: Método checkCNPJ [CNPJ "
                    + "(" + legalPerson.getCnpj() + ")" + " Válido:" + var + "]");
        } else {
