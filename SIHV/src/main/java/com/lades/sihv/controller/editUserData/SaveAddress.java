...
import com.lades.sihv.controller.address.*;
...
public class SaveAddress extends AbstractBean {
    ...
    public void methodSaveAddress(People person, VariablesAddress var) {
        System.out.println("►►►►►►►►►►►►► "
                + "SaveAddress > public void methodSaveAddress()");
        try {
            ...
            if (var.getHouse().getPkHouse() != null) {
                var.getHouse().setAddress(var.getObjAddress());
                getDaoGenerico().update(var.getHouse());
            } else {
                var.getHouse().setAddress(var.getObjAddress());
            ...
            System.err.println("►►►►►►►►►►►►► ERRO methodSaveAddress(): " + e.toString());
