...
CREATE TABLE IF NOT EXISTS `bd_sihv`.`animais` (
  ...
  `obitoAnimal` TINYINT(1) NOT NULL,
  `FK_cliente` INT NOT NULL,
  `FK_pessoa` INT NOT NULL,
  `cadDataHora` TIMESTAMP(6) NOT NULL,
  PRIMARY KEY (`PK_animal`, `FK_cliente`, `FK_pessoa`),
  INDEX `fk_animais_cliente1_idx` (`FK_cliente` ASC, `FK_pessoa` ASC),
  CONSTRAINT `fk_animais_cliente1`
    FOREIGN KEY (`FK_cliente` , `FK_pessoa`)
...
CREATE TABLE IF NOT EXISTS `bd_sihv`.`scheduling` (
  ...
  `registerClient` SET('Cadastrado', 'Novo') NOT NULL,
  `FK_animal` INT NULL,
  `FK_cliente` INT NULL,
  `FK_pessoa` INT NULL,
  `schedulingDate` TIMESTAMP(6) NOT NULL,
  PRIMARY KEY (`Pk_schedule`),
  INDEX `fk_scheduling_animais1_idx` (`FK_animal` ASC, `FK_cliente` ASC, `FK_pessoa` ASC),
  CONSTRAINT `fk_scheduling_animais1`
    FOREIGN KEY (`FK_animal` , `FK_cliente` , `FK_pessoa`)
    REFERENCES `bd_sihv`.`animais` (`PK_animal` , `FK_cliente` , `FK_pessoa`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `bd_sihv`.`consulta` (
  ...
  INDEX `fk_consulta_user1_idx` (`FK_user` ASC, `FK_pessoa` ASC),
  INDEX `fk_consulta_scheduling1_idx` (`FK_schedule` ASC),
  CONSTRAINT `fk_consulta_user1`
    FOREIGN KEY (`FK_user` , `FK_pessoa`)
    REFERENCES `bd_sihv`.`user` (`PK_user` , `FK_pessoa`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_consulta_scheduling1`
...
CREATE TABLE IF NOT EXISTS `bd_sihv`.`exameImage` (
  ...
  `solicitacaoData` TIMESTAMP(6) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `bd_sihv`.`species` (
  ...
  `dataSpeciesRegister` TIMESTAMP(6) NOT NULL,
  PRIMARY KEY (`PK_species`))
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `bd_sihv`.`races` (
  ...
  `FK_species` INT NOT NULL,
  `nameRaces` VARCHAR(100) NOT NULL,
  `dataRaceRegister` TIMESTAMP(6) NOT NULL,
  PRIMARY KEY (`PK_races`),
  INDEX `fk_races_species1_idx` (`FK_species` ASC),
  CONSTRAINT `fk_races_species1`
    FOREIGN KEY (`FK_species`)
    REFERENCES `bd_sihv`.`species` (`PK_species`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `bd_sihv`.`temporaryClientData`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `bd_sihv`.`temporaryClientData` (
  `PK_tempCliData` INT NOT NULL AUTO_INCREMENT,
  `proprietaryName` VARCHAR(100) NOT NULL,
  `animalName` VARCHAR(70) NOT NULL,
  `proprietaryPhones` VARCHAR(50) NOT NULL,
  `FK_schedule` INT NOT NULL,
  PRIMARY KEY (`PK_tempCliData`),
  INDEX `fk_temporaryClientData_scheduling1_idx` (`FK_schedule` ASC),
  CONSTRAINT `fk_temporaryClientData_scheduling1`
    FOREIGN KEY (`FK_schedule`)
    REFERENCES `bd_sihv`.`scheduling` (`Pk_schedule`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
START TRANSACTION;
USE `bd_sihv`;
INSERT INTO `bd_sihv`.`species` (`PK_species`, `nameSpecies`, `dataSpeciesRegister`) VALUES (1, 'Canino', DEFAULT);
INSERT INTO `bd_sihv`.`species` (`PK_species`, `nameSpecies`, `dataSpeciesRegister`) VALUES (2, 'Felino', DEFAULT);
...
START TRANSACTION;
USE `bd_sihv`;
INSERT INTO `bd_sihv`.`races` (`PK_races`, `FK_species`, `nameRaces`, `dataRaceRegister`) VALUES (1, 1, 'SRD', DEFAULT);
INSERT INTO `bd_sihv`.`races` (`PK_races`, `FK_species`, `nameRaces`, `dataRaceRegister`) VALUES (2, 2, 'SRD', DEFAULT);
